...
def getTotalArea(inputList, getFrom="coords", type="inside", R=1*u.m, units=u.m**2, angleUnits=u.rad):
    ...
    :param R: Radius of the sphere. Default is unit sphere R=1*u.m
    ...
    if getFrom == "theta":
        ...
        theta12, theta13, theta23 = getTriangleAngularDistances(inputList, units=angleUnits)

    if angleUnits is not None:
        piUse = pi * u.rad
        theta12 = theta12.to(u.rad)
        theta13 = theta13.to(u.rad)
        theta23 = theta23.to(u.rad)
    else:
        piUse = pi

    Omega_ti = R**2 * (theta12 + theta13 + theta23 - piUse)
