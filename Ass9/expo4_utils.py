...
####################################################################################################################
# Functions to reduce gamma search space
...
######################################################################################################################
# Functions to find intermediary values and such
...
def getdt(dtheta, dtdtheta, outputUnits=None):
    #TODO: might ditch this function. Is deprecated
    """
...
######################################################################################################################
# Functions to find TOF and to abstractify previous functions into higher level ones
...
def doTOFCalcMain(gamma1s, stepNumbers, k2, r1, r2, Psi, N, mu, saveName="test.npy", TOFArrUnits=u.year, printing=True):
    ...
    for i in range(len(gamma1s)):
        for j in range(len(stepNumbers)):
            ...
            if printing: print("gamma %s, step %s. percent completion: %s" %
                               (np.round(gamma1, decimals=4), int(stepNumber), int(percent)))
